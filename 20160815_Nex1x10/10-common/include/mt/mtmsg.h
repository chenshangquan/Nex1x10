 ...
 * @date    20140404
 ---------------------------------------------------------------------------*/
class MTCBB_API CMtMsg
{	
	struct TBodyItem
	{
		BOOL32 bDelete; ///<CMtMsg内部使用，用于决定是否在CMtMsg对象销毁时delete tBody
...
public:
	...
	* SetUserData
	* 
	* @brief   设置用户自定义数据（CMtMsg仅透传该字段，该字段的含义由用户自己解释）
	* 
...
* @return  BOOL32 成功返回TRUE，失败返回FALSE
------------------------------------------------------------------------*/
//lint -esym(714, mtmsg::InitMtMsgLib)
MTCBB_API BOOL32 InitMtMsgLib();
...
* @return  BOOL32 成功返回TRUE，失败返回FALSE
------------------------------------------------------------------------*/
//lint -esym(714, mtmsg::UnInitMtMsgLib)
MTCBB_API BOOL32 UnInitMtMsgLib();
...
------------------------------------------------------------------------*/
template<class T>
MTCBB_API const s8* GetEnumVauleName(s32 nValue)
{
...
------------------------------------------------------------------------*/
MTCBB_API BOOL32 IsCmdMsg(std::string strMsgId);
...
------------------------------------------------------------------------*/
MTCBB_API BOOL32 IsNtfMsg(std::string strMsgId);
...
------------------------------------------------------------------------*/
MTCBB_API BOOL32 IsReqMsg(std::string strMsgId);
...
------------------------------------------------------------------------*/
MTCBB_API BOOL32 IsRspMsg(std::string strMsgId);
