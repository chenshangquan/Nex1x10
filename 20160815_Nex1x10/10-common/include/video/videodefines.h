...
typedef enum
{
    VID_PAL_MODE = 25,     //PAL制式,每秒25帧
    VID_NTSC_MODE = 30,        //NTSC制式,每秒30帧
...
#define ENC_QUALITY_LEVEL_10         (1 << 15) //MP编码单线程 
...
#define  NV21		42021
#define  YV12     4202    //分三个平面，Y平面，U平面，V平面，其中U平面和V平面均为Y平面的1/4  Y0 Y1 Y2 Y3.. U0 U1.. V0 V1..
...
typedef enum
{
	...
	IMG_WATERMARKING,         //图像数字水印模块
	IMG_LOGO_WATERMARKING,     //图像logo和数字水印模块
	IMG_BMPTOYUVALPHA_WM,       //BMP图像转为YUV图像并生成Alpha通道，用于数字水印
...
typedef enum
{
    ...
    PIC_MERGE_ZOOM_M0 = 20000,              //无画面合成  23
    PIC_MERGE_ZOOM_M1 = 20100,            //一画面合成(原PIC_MERGE_ONE) 24
    PIC_MERGE_ZOOM_M2 = 20200,            //两画面合成(原PIC_MERGE_VTWO) 25
    PIC_MERGE_ZOOM_M2_1_BR1,            //两画面合成(一大一小，小画面在大画面内的右下角，原PIC_MERGE_ITWO) 26
    PIC_MERGE_ZOOM_M2_1_BL1,          //两画面合成(一大一小，小画面在大画面内的左下角）
    PIC_MERGE_ZOOM_M2_1_TR1,          //两画面合成(一大一小，小画面在大画面内的右上角）
    PIC_MERGE_ZOOM_M2_1_TL1,          //两画面合成(一大一小，小画面在大画面内的左上角）
    PIC_MERGE_ZOOM_M3_T1 = 20300,         //三画面合成(大小相等，一上两下，原PIC_MERGE_THREE) 27
    PIC_MERGE_ZOOM_M3_B1,               //三画面合成(大小相等，两上一下)  28
    PIC_MERGE_ZOOM_M3_1_B2,             //三画面合成(一大两小，小画面在下) 29
    PIC_MERGE_ZOOM_M3_1_T2,             //三画面合成(一大两小，小画面在上)  30
    PIC_MERGE_ZOOM_M3_1_R2,             //三画面合成(一大两小，左大画面，右小画面) 31
    PIC_MERGE_ZOOM_M3_L1,    //三画面合成(大小相等，一左两右）---new  32
	PIC_MERGE_ZOOM_M3_L2,    //三画面合成(大画面为背景2/3小画面位背景1/3，一左两右）---new  32_1
    PIC_MERGE_ZOOM_M4 = 20400,            //四画面合成(原PIC_MERGE_FOUR) 33
    PIC_MERGE_ZOOM_M4_1_R3,             //四画面合成(一大三小，小画面在右，原PIC_MERGE_SFOUR) 34
    PIC_MERGE_ZOOM_M4_1_D3,//四画面合成(一大三小，小画面在下面)   35
    PIC_MERGE_ZOOM_M5_1_R4 = 20500,       //五画面合成(一大四小，小画面在右)  36
    PIC_MERGE_ZOOM_M5_1_D4,             //五画面合成(一大四小，小画面在下)  37
    PIC_MERGE_ZOOM_M5_2_D3,                                               //  38
    PIC_MERGE_ZOOM_M6 = 20600,            //六画面合成(大小相等，上三下三)   39
    PIC_MERGE_ZOOM_M6_1_5,              //六画面合成(一大五小，原PIC_MERGE_SIX) 40
    PIC_MERGE_ZOOM_M6_2_B4,             //六画面(两大四小，小画面在下)    41
    PIC_MERGE_ZOOM_M6_1_B5,             //六画面(一大五小，小画面在下)    42
    PIC_MERGE_ZOOM_M6_B5,
    PIC_MERGE_ZOOM_M7_3_TL4 = 20700,      //七画面合成(三大四小，四个小画面在左上角) 43
    PIC_MERGE_ZOOM_M7_3_TR4,            //七画面合成(三大四小，四个小画面在右上角)  44
    PIC_MERGE_ZOOM_M7_3_BL4,            //七画面合成(三大四小，四个小画面在左下角)  45
    PIC_MERGE_ZOOM_M7_3_BR4,            //七画面合成(三大四小，四个小画面在右下角，原PIC_MERGE_SEVEN)  46
    PIC_MERGE_ZOOM_M7_3_BLR4,           //七画面合成(三大四小，四个小画面在下面左右两侧)  47
    PIC_MERGE_ZOOM_M7_3_TLR4,           //七画面合成(三大四小，四个小画面在上面左右两侧)  48
    PIC_MERGE_ZOOM_M7_1_D6,
    PIC_MERGE_ZOOM_M8_1_7 = 20800,        //八画面合成(一大七小，原PIC_MERGE_EIGHT)  49
    PIC_MERGE_ZOOM_M8_4_4,              //50
    PIC_MERGE_ZOOM_M9 = 20900,            //九画面合成(原PIC_MERGE_NINE)  //51
    PIC_MERGE_ZOOM_M9_T4_1_D4,                                            //52
    PIC_MERGE_ZOOM_M10_2_R8 = 21000,     //十画面合成(两大八小，八个小画面在右侧，原PIC_MERGE_TEN) 53
    PIC_MERGE_ZOOM_M10_2_B8,            //十画面合成(两大八小，八个小画面在下面，原PIC_MERGE_TEN_2_8) 54
    PIC_MERGE_ZOOM_M10_2_T8,            //十画面合成(两大八小，八个小画面在上面)  55
    PIC_MERGE_ZOOM_M10_2_L8,            //十画面合成(两大八小，八个小画面在左侧)  56
    PIC_MERGE_ZOOM_M10_2_TB8,           //十画面合成(两个大画面居中，上下各四个小画面）57
    PIC_MERGE_ZOOM_M10_1_9,             //十画面合成(一大九小，原PIC_MERGE_TEN_1_9)    58
    PIC_MERGE_ZOOM_M10_L4_2_R4,         //59
    PIC_MERGE_ZOOM_M11_T5_1_D5 = 21100,  //60
    PIC_MERGE_ZOOM_M11_1_D10,           //61
    PIC_MERGE_ZOOM_M12_1_11 = 21200,     //十二画面合成(一大十一小，原PIC_MERGE_TWELVE) 62
    PIC_MERGE_ZOOM_M12_3_RD9,           //63
    PIC_MERGE_ZOOM_M13_TL1_12 = 21300,   //十三画面合成(大画面在左上角，PIC_MERGE_THIRTEEN) //64
    PIC_MERGE_ZOOM_M13_TR1_12,          //十三画面合成(大画面在右上角) 65
    PIC_MERGE_ZOOM_M13_BL1_12,          //十三画面合成(大画面在左下角) 66
    PIC_MERGE_ZOOM_M13_BR1_12,          //十三画面合成(大画面在右下角) 67
    PIC_MERGE_ZOOM_M13_1_ROUND12,       //十三画面合成(大画面居中，小画面环绕四周) 68
    PIC_MERGE_ZOOM_M13_TL4_9,           //69
    PIC_MERGE_ZOOM_M13_L6_1_R6,         //70
    PIC_MERGE_ZOOM_M14_1_13 = 21400,     //十四画面合成(一大十三小，原PIC_MERGE_FOURTEEN) 71
    PIC_MERGE_ZOOM_M14_TL2_12,          //十四画面合成(两个大画面在左上角) 72
    PIC_MERGE_ZOOM_M14_T5_1_2_1_D5,     //73
    PIC_MERGE_ZOOM_M15_T3_12 = 21500,    //十五画面合成(上三下十二) 74
    PIC_MERGE_ZOOM_M15_T4_L3_1_R3_D4,   //75
    PIC_MERGE_ZOOM_M16 = 21600,          //十六画面合成(原PIC_MERGE_SIXTEEN) 76
    PIC_MERGE_ZOOM_M16_1_15,            //十六画面合成(一大十五小，原PIC_MERGE_SIXTEEN_1_15) 77
    ...
    PIC_MERGE_ZOOM_M25_1 = 22500,   //93       //二十五画面合成(原PIC_MERGE_TWENTYFIVE)
    PIC_MERGE_ZOOM_M25_2,          //94
    PIC_MERGE_ZOOM_M36 = 23600,          //三十六画面合成(原PIC_MERGE_THIRTYSIX)
    PIC_MERGE_ZOOM_M49 = 24900,          //四十九画面合成(原PIC_MERGE_FORTYNINE)
    PIC_MERGE_ZOOM_M64 = 26400           //六十四画面合成(原PIC_MERGE_SIXTYFOUR)
} PicMergeZoomStyle;
...
typedef enum
{
	...
	ENC_OUTPUT_MODE,	 // 设置编码器输出模式(网络传输, 本地保存)
	ENC_DYNAMIC_KEY_FRAME, // 设置编码器动态关键帧
    ENC_SLICE_ALIGN,     // 设置Slice四字节对齐
    ENC_VA_PARAM,        // VA编码参数扩展(仅用于Linux下VA H.264编码)
    ENC_USER_INFO,        // 用户自定义信息扩展
    ENC_RTP_FLAG,         //设置是否开启RTP打包
    ENC_ROI_FLAG,          //设置是否开启ROI
	ENC_SET_THREAD_PRI	  //设置线程优先级
...
#define ERR_VID_H265ENC_MOD (17 << 22)    //h265编码器子模块
#define ERR_VID_H265DEC_MOD (18 << 22)    //h265解码器子模块
#define ERR_VID_SVACENC_MOD (19 << 22)    //SVAC编码器子模块
#define ERR_VID_QUERY_MOD  (20 << 22)  //VideoQuery子模块
#define ERR_VID_SVACDEC_MOD (21 << 22)    //SVAC编码器子模块
#define ERR_VID_SVAC2ENC_MOD (22 << 22)    //SVAC编码器子模块
#define ERR_VID_SVAC2DEC_MOD (23 << 22)    //SVAC编码器子模块
...
#define ERR_IMG_BMPTOYUV_ALPHA_WM_MOD (41 << 22) //BMP转为YUV并生成Alpha通道模块(用于数字水印)
#define ERR_IMG_WATERMARKING_MOD (42 << 22) //数字水印模块
