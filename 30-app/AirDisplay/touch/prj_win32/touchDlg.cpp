...
void CapScreenVidStart( void *pFrame, u32 dwContext )
{
	...
	if (stFrameHdr.m_tVideoParam.m_bKeyFrame)
	{
		...
		{
			PRINTMSG_TIME( "CapScreenVidStarts回调:收到第一个关键帧\r\n");
		}
    ...
    //判断是否已超过最大堆积包数
    if ( g_dlg->GetVideoDataList()->Size() > MAX_STORE_NUM )
    {
        PRINTMSG_TIME( "Video Buffer Full, Clear DataList\r\n" );
        g_dlg->GetVideoDataList()->Clear();

        //判断音视频发送线程是否异常
        if ( !g_dlg->m_bVideoThreadRun || !g_dlg->m_bAudioThreadRun)
        {
            PRINTMSG_TIME("AVThread Abnormal, m_bVideoThreadRun:%d,m_bAudioThreadRun:%d\r\n",
                g_dlg->m_bVideoThreadRun, g_dlg->m_bAudioThreadRun);
        }

        //判断业务是否挂掉
        if ( g_dlg->m_bBusinessStaus && g_dlg->m_bIsProjecting )
        {
...
void VideoCapStdCB(u32 dwWidht, u32 dwHeight, u32 dwContext)
{
    ...
    {
        //实际分辨率小于协商分辨率，取实际分辨率为当前编码分辨率
        if ( dwWidht <= g_dlg->m_tVideoEncParam.m_wEncVideoWidth
            && dwHeight <= g_dlg->m_tVideoEncParam.m_wEncVideoHeight )
        {
    ...
    PRINTMSG("SetVideoEncParam, wEncVideoWidth:%d,wEncVideoHeight:%d\r\n",
        tVideoEncParam.m_wEncVideoWidth, tVideoEncParam.m_wEncVideoHeight);
...
CString UTF8_To_CString(const std::string& str) 
{
