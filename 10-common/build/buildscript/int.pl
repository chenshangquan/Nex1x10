...
print "!!!!!*****$version_name!!!!!!\n";
    ...
    print OUTFILE  $UCMprj."_".$version."_R".$version_name."#T".$gettime;
    close(OUTFILE);
...
###根据发布规则在发布路径后加日前文件夹/日前时间文件夹以区分每次发布;20151225-yeshufang增加SVN版本号 
###$RP = &revisepath(1,$ReleaseP.$builddatetime."_R".$version_name); 

###20151228-yeshufang modify发布文件夹名称,格式为：Date/ProjectName_Rxxxxx#T时间,例如:20151228\mediacontrol_R2055#T20151228-1130
###$RP = &revisepath(1,$ReleaseP.$builddate."\\".$DynviewN."_R".$version_name."#T".$builddatetime); 

###20151228-yeshufang modify发布文件夹名称,格式为：Date/ProjectName_Version_Rxxxxx#T时间,例如:20151228\mediacontrol_daily_R2055#T1130
$RP = &revisepath(1,$ReleaseP.$builddate."\\".$UCMprj."_".$version."_R".$version_name."#T".$gettime); 
			...
			if ( $value[0] =~ /^$str$/i ) # 必须当share.txt中的操作系统标识与当前操作系统相反时,才确认其值可能有效.
			{
				...
				else
				{
					if ( $value[1] =~ /^\d{8}-\d{4}$/ ) # 日期符合yyyymmdd-hhmm格式
					{
						...
						my $dis = &difftime($gettime,$synctime[1]);
						$dis = abs($dis);
