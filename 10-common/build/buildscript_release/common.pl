...
# $gettime   : Current hhmm
# $gen_win_revision : get svn version from windows
# $gen_linux_revision : get svn version from linux
...
sub getenv
{
	...
	foreach $envkey (@envkeys) # 逐个处理传入的环境关键字
	{
		...
		if ( @found == 0 ) # 没有找到环境关键字的行
		{
			...
			if (( $envkey =~ /^LinuxCC$/i ) ||( $envkey =~ /^SVN$/i )|| ( $envkey =~ /^GROUPS$/i ) || ( $envkey =~ /^DEFENV$/i ))
			{
		...
		elsif (( $envkey =~ /^PPC[\d\._]*$/i )||( $envkey =~ /^PPC_8377[\d\._]*$/i ) ||( $envkey =~ /^PPC_QORID[\d\._]*$/i )|| ( $envkey =~ /^ARM[\d\._]*$/i ) ||( $envkey =~ /^ARM_NONE[\d\._]*$/i )|| ( $envkey =~ /^EQT[\d\._]*$/i ) || ( $envkey =~ /^DAVINCI[\d\._]*$/i )|| ( $envkey =~ /^HISI[\d\._]*$/i ))
		{
			foreach $f (@found) # 逐行处理环境关键字的行
			{
				my @value = &getvalue("=",$f); # 分离环境变量value[0]与环境变量值value[1]
...
###			}
			...
			close(BUILDINFO);
			#######发布repo编译代码点记录xml文件
			my $repo_tag=`repo manifest -o - -r | grep xml`;
			if ( $repo_tag ne "" )
			{	
				system("repo manifest -o - -r > $verworkP$builddatetime'.xml';pwd;");
				print "NOW is  repo mainifiet........<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<<..........";	
			}			           
			# 发布编译信息文件
			if ( $options =~ /r/ )
			{
				...
				if ( $repo_tag ne "" )
				{	
					&command("copy","f",$verworkP.$builddatetime.".xml",$RP);
					print "NOW is Copying  repo mainifiet xml......>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>..........";
				}
				#####hanjian 20120810 
		...
		if ( open(BUILDPATH,">$workpath"."ReleasePath.log") )
		{
		 ...
		 close(BUILDPATH);
		 &copyrelpath($workpath,$SnapviewP);
		}
	}
...
sub command
{
	...
	elsif ( $cmd eq "revert" )
	{
		...
		{
				$ENV{LANG} = "en_US.UTF-8";
        	    #system("svn revert  --username buildnvr --password 'build21' --no-auth-cache $item0  >$item1");####--- modify by hanjian 20120821  for update 
		    #system("svn revert  --username root --password 'kdckdc' --no-auth-cache -R $item0 2>$vererrL");
		}
		else
		{
			  #system("svn revert $item0  >$item1");####--- modify by hanjian 20120821  for update 
		    #system("svn revert -R $item0 2>$vererrL");
		}
	...
	elsif ( $cmd eq "copy" )
	{
		...
		if ( $OS )
		{
			...
			if ( $os_var =~ /linux/i )
			{
				...
				system("sudo mkdir -p $item2 2>$vererrL") if ( !-e $item2 );
				if ($item0 eq "d"){
					system("sudo cp -R -P  $item1 $item2 2>$vererrL");
				}else{ # if ($item0 eq "f")
					system("sudo cp -f -P  $item1 $item2 2>$vererrL");
				}
			}
		}
		else
...
#获取windows编译时的svn版本号
sub gen_win_revision
{
   chdir $SnapviewP;
   print "The path is : $SnapviewP";
   #system("svn update  --accept theirs-full --username build90 --password kdv40build08 --no-auth-cache");
   my $svninfo=`svn info .| findstr  Rev:`;
   if ( $svninfo eq "" )
   {
		my @gitrev=`git log -n 1`;
		print $gitrev[0];
	   ##print "33333333333\n";
	   @gitrev_i=split/\s/,$gitrev[0];
	   $gitrev_value=$gitrev_i[1];
	   $gitrev_value=~s/^ +//;  ###去除字符串前面的空格
	   print "==========\n";
	   print $gitrev_value;     ###输出版本号
	   print "\n";
	   my $rev_value=substr($gitrev_value,0,6);
	   print $rev_value;
	   print "\n";
	   print "==========\n";
   }
   ...
   {
       ##print "33333333333\n";
       @svnrev = split /\s+/,$svninfo;
       $rev_value = $svnrev[-1];
       ##print "=====111=====\n";
       print $rev_value;     ###输出版本号
       ##print "=====222=====\n";
   }  
   return $rev_value;
}
...
#获取linux编译时的svn版本号
sub gen_linux_revision
{
   chdir $SnapviewP;
   print "The path is: $SnapviewP\n";
   #system("svn update  --accept theirs-full --username root --password 'kdckdc' --no-auth-cache");
   my $svninfo=`svn info .| grep  Rev:`;
   ##print "33333333333\n";
   if ( $svninfo eq "" )
   {
		my @gitrev=`git log -n 1`;
...
{	
	...
	$item2 = $item0.'ReleasePath.log';
	print  $item2
	print  $item1;
	if ( $OS )
	{
		system("cp -rvf  $item2  $item1");
	}
	else
	{			
		system("xcopy /Y $item2  $item1");		
	}
}
sub gettikid
{	
	my ($item0) = @_;
	print "It's getting ticketID........................";
	@tikid = split(/-/,$item0);
	return @tikid[0];
}
# =====================================================================================================================
