...
if ( $spoptions =~ /r/ )
{
	...
	if ( $spspecRP eq "" ) # 当用户没有指定发布路径 , 根据发布规则在发布路径后加日前文件夹/日前时间文件夹以区分每次发布
	{
		...
		##my $p_i=$builddate."\\".$DynviewN."_R".$version_name."#T".$builddatetime; #20151228-yeshufang modify发布文件夹名称,格式为：Date/ProjectName_Rxxxxx#T时间,例如:20151228\mediacontrol_R2055#T20151228-1130
		##my $p_i=$builddate."\\".$DynviewN."_R".$version_name."#T".$gettime;  #20151228-yeshufang modify发布文件夹名称,格式为：Date/ProjectName_Rxxxxx#T时间,例如:20151228\mediacontrol_R2055#T1130
		if (($ticketID eq "") || ($ticketID =~ /^N/))
		{
			...
			if ( $version_name eq "" )
			{
				...
				our $p_i=$builddate."-SP"."\\".$UCMprj."_".$version."_R".$version_name."#T".$gettime;  #20151228-yeshufang modify发布文件夹名称,格式为：Date/ProjectName_Version_Rxxxxx#T时间,例如:20151228\mediacontrol_daily_R2055#T1130
	#			print $p_i
			}			
		}
		else
