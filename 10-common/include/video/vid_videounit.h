...
typedef struct
{
    u32 u32EncReservedType; // 编码参数扩展类型(ENC_OUTPUT_MODE)
	l32 l32EncOutputMode;   // 编码输出模式 (0: 网络传输 1: 本地保存)
    u32 u32Reserved;        // 保留参数
}TEncOutputMode;

typedef struct
{
    u32 u32EncReservedType;	    // 编码参数扩展类型(ENC_DYNAMIC_KEY_FRAME)
    l32 l32EncDynamicKeyFrame;  // 编码动态关键帧
	u32 u32Reserved;            // 保留参数
}TEncDynamicKeyFrame;
...
typedef struct
{
    u32  u32EncExtendType;      //扩展类型(ENC_RTP_FLAG)
    u32  u32NoRtpFlag;          //是否开启RTP头（1：关闭，0：开启）
    u32  u32Reserved;           //保留参数
}TEncRTPFlagParam;
...
typedef struct
{
    u32 u32EncReservedType;	// 编码参数扩展类型(ENC_SET_THREAD_PRIORITY)
    l32 l32EnableThreadPri;	// 使能线程优先级
    u32 u32Reserved;		// 保留参数
}TEncSetThreadPri;
...
typedef struct
{
    u32 u32EncExtendType;        //扩展类型(ENC_ROI_FLAG)
    l32 l32EnableROI;            //ROI使能标记位（1：开启, 0:关闭）
    l32 l32NonRoiSkipFlag;       //非ROI区域是否跳过，只支持0(0: background not skipped, 1: background skipped) 
    u32 u32TopLeftX;             //ROI区域左上位置（坐标）
    u32 u32TopLeftY;             //ROI区域左上位置
    u32 u32BottomRightX;         //ROI区域右下位置
    u32 u32BottomRightY;         //ROI区域右下位置
    u32 u32SegmentID;            //ROI区域SegmentID（0-7）
    u8 u8SegmenQP;               //ROI区域QP（暂未启用）
    u32 u32Reserved;             //保留参数
}TEncRoiParam;
...
typedef struct	// PTZ转动标记
{
	u32 u32ReservedType;	// 编码器输入扩展类型(ENC_YUV_BUFFER_INPUT)
	u8  u8PTZFlag;			// PTZ转动标记
	u32 u32Reserved;        // 保留参数
	u64 u64Reserved;		// 保留参数
}TEncPTZFlag;
...
typedef struct
{
	...
	void *vInResParam;         //保留输入参数指针
	u8 u8FlushDecodeFlag;
    u8 u8InsertFrameFlag;
	u32 u32Reserved;		//保留参数
    u64 u64Reserved;		//保留参数
}TDecInput;
...
typedef struct
{
    ...
    u8 u8OutIsDelay;
	void *vOutResParam;  //保留输出参数指针
	u8 u8YUVIsAllOutput;
    void *pvMVContext;    //MV信息
    u32	u32Reserved;	 //保留参数
    u64 u64Reserved;
}TDecOutput;
...
typedef struct
{
    u8 u8FrameType;          //解码器当前帧类型
    s8 s8Valued;             //当前块MV是否有效
    s16 s16MVx;              //X方向
    s16 s16MVy;              //Y方向
}TDecOutMVInfo;
