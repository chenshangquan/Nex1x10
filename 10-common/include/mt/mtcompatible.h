...
#ifndef _MT_H323STACKCOMPATIBLE_H
#define _MT_H323STACKCOMPATIBLE_H
...
//mcs显示终端视频源别名功能
#define    MT_MAXNUM_VIDSOURCE        (u8)(10)            //最大视频源数
#define    MT_MAX_PORTNAME_LEN        (u8)(16)            // 视频矩阵端口名最大长度           
...
#define ROLL_HEAD_LEN		11
#define ROLLTYPE_OFFSET	    0
#define ROLLTYPE_LEN		1
#define ROLLTIME_OFFSET		(ROLLTYPE_OFFSET + ROLLTYPE_LEN)
#define ROLLTIME_LEN		1
#define ROLLRATE_OFFSET		(ROLLTIME_OFFSET + ROLLTIME_LEN)
#define ROLLRATE_LEN		1
#define SRCMT_OFFSET		(ROLLRATE_OFFSET + ROLLRATE_LEN)
#define SRCMT_LEN			2
#define DSTMTNUM_OFFSET		(SRCMT_OFFSET + SRCMT_LEN)
#define DSTMTNUM_LEN		2
#define ROLLMSGLEN_OFFSET	(DSTMTNUM_OFFSET + DSTMTNUM_LEN)
#define ROLLMSGLEN_LEN		2
#define WholeRollMsg_OFFSET		(ROLLMSGLEN_OFFSET + ROLLMSGLEN_LEN)
#define WholeRollMsg_Len	    2
#define ROLL_BODY_OFFSET		ROLL_HEAD_LEN
//
//  矩阵信息
//
...
#define SAFE_PATIBLE_MEMCPY(pDst, dwDstSize, pSrc, dwSrcSize)\
do \
{\
	if (NULL== (pDst) || NULL == (pSrc) || (dwDstSize) <= 0 || (dwSrcSize) <= 0)\
	{\
	   mtPrintf("SAFE_PATIBLE_MEMCPY Invalid Param(pDst:%0x,dwDstSize:%d,pSrc:%0x,dwSrcSize:%d)(%s:(%d))!", pDst, dwDstSize, pSrc, dwSrcSize, __FILE__, __LINE__);  \
    }\
    else\
	{\
	if ((dwDstSize) < (dwSrcSize))\
		...
		mtPrintf("SAFE_PATIBLE_MEMCPY dwDstSize(%d) < dwSrcSize(%d) (%s:(%d))!", dwDstSize, dwSrcSize, __FILE__, __LINE__);\
	}\
		else\
		{\
...
public :
	CRollMsg()
	{
		...
		u16 wLen = ROLL_HEAD_LEN;
		wLen = htons( wLen );
		
		SAFE_PATIBLE_MEMCPY( m_abyBuffer+WholeRollMsg_OFFSET, sizeof(m_abyBuffer)-WholeRollMsg_OFFSET, 
			                 &wLen, WholeRollMsg_Len );
...
/*	====================================================================
	...
	void SetMsgId( const u8 &byMsgId )
	{
		...
		u16 wOffSet = ROLL_HEAD_LEN + GetDstMtNum() * sizeof(TMTLABEL) + GetRollMsgContentLen();
		...
		SAFE_PATIBLE_MEMCPY( m_abyBuffer + WholeRollMsg_OFFSET, sizeof(m_abyBuffer)-WholeRollMsg_OFFSET,
			                 &wTotalLen,WholeRollMsg_Len );

// 		memcpy( m_abyBuffer + WholeRollMsg_OFFSET, &wTotalLen,
// 			WholeRollMsg_Len );
	}
...
/*	====================================================================
	...
	u8 GetMsgId(void)
	{
		u16 wOffSet = ROLL_HEAD_LEN + GetDstMtNum() * sizeof(TMTLABEL) + GetRollMsgContentLen() + 1;
	...
	*/
	void SetType( u8 byMsgType ) 
	{
		...
		*( m_abyBuffer + ROLLTYPE_OFFSET ) = byMsgType;
		return;
	}
	...
	====================================================================
	*/
	u8 GetType( void ) { return *( m_abyBuffer + ROLLTYPE_OFFSET ); }
	...
	*/
	void SetRollTimes( u8 byTimes ) 
	{
		if ( byTimes == 0 )
			byTimes = 1;	// 保证至少滚动一次
		*( m_abyBuffer + ROLLTIME_OFFSET ) = byTimes; 
	}
	/*
	====================================================================
	功能        ：获取滚动次数
	...
	====================================================================
	*/
	u8 GetRollTimes( void ) { return *( m_abyBuffer + ROLLTIME_OFFSET ); }
	...
	====================================================================
	*/
	void SetRollRate( u8 byRate ) { *( m_abyBuffer + ROLLRATE_OFFSET ) = byRate; }
	/*
	====================================================================
	功能        ：获取滚动速度
	输入参数说明：无
	...
	====================================================================
	*/
	u8 GetRollRate( void ) { return *( m_abyBuffer + ROLLRATE_OFFSET ); }
	...
	*/
	...
	{
		*( m_abyBuffer + SRCMT_OFFSET ) = byMcuNo;
		*( m_abyBuffer + SRCMT_OFFSET + 1 ) = byMtNo;
	}
	...
	*/
	...
	{
		TMTLABEL tMsgSrc;
		u8 byMcuNo = *( m_abyBuffer + SRCMT_OFFSET );
		u8 byMtNo = *( m_abyBuffer + SRCMT_OFFSET + 1 );
	...
	*/
	...
	{ 
		u16 wMtNum = 0;
// 		memcpy( &wMtNum, m_abyBuffer + DSTMTNUM_OFFSET,
// 			DSTMTNUM_LEN );
		SAFE_PATIBLE_MEMCPY( &wMtNum, sizeof(wMtNum), m_abyBuffer + DSTMTNUM_OFFSET, DSTMTNUM_LEN );
...
// 			u16 wOffSet = ROLL_BODY_OFFSET + wDstMtNum * sizeof( TMTLABEL );
// 			memcpy( abyRollMsg,
// 					m_abyBuffer + ROLL_BODY_OFFSET, 
// 					min( MAX_ROLLMSG_LEN, wRollMsgLen ) );
// 			memcpy( m_abyBuffer + wOffSet, abyRollMsg, min( MAX_ROLLMSG_LEN, wRollMsgLen ) );
// 		}
// 		memcpy( m_abyBuffer + ROLL_BODY_OFFSET, ptMtList, wDstMtNum * sizeof( TMTLABEL ) );
// 
// 		// 总的消息长度
// 		u16 wMsgLen = wRollMsgLen + wDstMtNum * sizeof( TMTLABEL ) + ROLL_HEAD_LEN;
// 		wMsgLen = htons( wMsgLen );
// 		memcpy( m_abyBuffer + WholeRollMsg_OFFSET, &wMsgLen,
// 				WholeRollMsg_Len );
// 
// 		// 终端列表长度
// 		wDstMtNum = htons( wDstMtNum );
// 		memcpy( m_abyBuffer + DSTMTNUM_OFFSET, &wDstMtNum,
// 			DSTMTNUM_LEN );
// 
	...
	*/
	void AddDstMt( TMTLABEL tMt )
	{
		...
		// 总的消息长度
		u16 wMsgLen = wRollMsgLen + ( wDstMtNum + 1 )* sizeof( TMTLABEL ) + ROLL_HEAD_LEN;
		wMsgLen = htons( wMsgLen );
		
		SAFE_PATIBLE_MEMCPY(m_abyBuffer + WholeRollMsg_OFFSET, sizeof(m_abyBuffer)-WholeRollMsg_OFFSET,&wMsgLen, WholeRollMsg_Len  );
		...
		*( m_abyBuffer + wOffSet + 1 ) = tMt.GetMtNo();
		...
		SAFE_PATIBLE_MEMCPY( m_abyBuffer + DSTMTNUM_OFFSET, sizeof(m_abyBuffer)-DSTMTNUM_OFFSET, &wDstMtNum, DSTMTNUM_LEN );
	...
	====================================================================
	*/
	TMTLABEL *GetDstMtList( void ) { return ( TMTLABEL* )( m_abyBuffer + ROLL_BODY_OFFSET ); }
	...
	*/
	int GetFirstDstMt( TMTLABEL *ptMt = NULL ) 
	{
		...
		if ( GetDstMtNum() != 0 )
		{
			ptMt->SetMtLabel( *( m_abyBuffer + ROLL_BODY_OFFSET ),
							*( m_abyBuffer + ROLL_BODY_OFFSET + 1 ) );
		}
	...
	*/
	int GetNextDstMt( u16 wIndex, TMTLABEL *ptMt = NULL )
	{
		...
		u16 wOffSet = ROLL_BODY_OFFSET + ( wIndex + 1 ) * sizeof( TMTLABEL );
	...
	*/
	u16 GetRollMsgContentLen( void )
	{
		...
		SAFE_PATIBLE_MEMCPY( &wMsgLen, sizeof(wMsgLen), m_abyBuffer + ROLLMSGLEN_OFFSET, ROLLMSGLEN_LEN );

// 		memcpy( &wMsgLen, m_abyBuffer + ROLLMSGLEN_OFFSET,
// 				ROLLMSGLEN_LEN );
	...
	*/
	void SetRollMsgContent( const u8* pbyRollMsgContent = NULL, u16 wRollMsgLen = 0 )
	{
		...
		// 终端列表
		u16 wDstMtNum = GetDstMtNum();
		u16 wlen = ROLL_BODY_OFFSET + wDstMtNum * sizeof( TMTLABEL );

		SAFE_PATIBLE_MEMCPY(m_abyBuffer + wlen, sizeof(m_abyBuffer)-wlen,  pbyRollMsgContent, wRollMsgLen );

// 		memcpy( m_abyBuffer + ROLL_BODY_OFFSET + wDstMtNum * sizeof( TMTLABEL ),
...
// 			WholeRollMsg_Len );
		...
		SAFE_PATIBLE_MEMCPY(m_abyBuffer + ROLLMSGLEN_OFFSET, sizeof(m_abyBuffer)-ROLLMSGLEN_OFFSET,&wRollMsgLen, ROLLMSGLEN_LEN );

// 		memcpy( m_abyBuffer + ROLLMSGLEN_OFFSET, &wRollMsgLen,
// 				ROLLMSGLEN_LEN );
	}
	...
	*/
	...
	{
		u16 wDstMtNum = GetDstMtNum();
		return ( m_abyBuffer + ROLL_BODY_OFFSET + wDstMtNum * sizeof( TMTLABEL ) ); 
	}
	...
	*/
	...
	{ 
		...
		SAFE_PATIBLE_MEMCPY(&wMsgLen, sizeof(wMsgLen),m_abyBuffer + WholeRollMsg_OFFSET, WholeRollMsg_Len );

// 		memcpy( &wMsgLen, m_abyBuffer + WholeRollMsg_OFFSET,
// 				WholeRollMsg_Len );
	...
	*/
	...
	{
		if ( wMsgLen < ROLL_HEAD_LEN )
			return FALSE;

		wMsgLen = min( wMsgLen, sizeof( m_abyBuffer ) );
		SAFE_PATIBLE_MEMCPY( m_abyBuffer, sizeof(m_abyBuffer), pbyMsg, wMsgLen );
...
// 		memcpy( m_abyBuffer + WholeRollMsg_OFFSET,
// 			    &wLen, WholeRollMsg_Len );
	}
...
public :
	...
	{ 
		...
		SAFE_PATIBLE_MEMCPY( achAlias, sizeof(achAlias), pchName, byLen );
...
public:
	...
	BOOL32   operator ==( const TMt & tObj ) const;                  //判断是否相等
...
public:
	...
	BOOL32 operator ==( const TMtAlias & tObj ) const;
...
public:
    ...
    void   SetSpecMixing(BOOL32 bMixSpecMt = TRUE)
    {
        u8 byMixMode = mcuNoMix;
        if ( bMixSpecMt )
        {
...
public:
	...
	void SetVideoCap( TVideoStreamCap &tVidCap ){ SAFE_PATIBLE_MEMCPY( &m_tVideoCap, sizeof(m_tVideoCap), &tVidCap, sizeof(TVideoStreamCap) ); }//memcpy(&m_tVideoCap, &tVidCap, sizeof(TVideoStreamCap)); }
...
public:
	...
	//第一路音视频能力集设置
    void    SetMainSimCapSet( TSimCapSet &tSimCapSet ) { SAFE_PATIBLE_MEMCPY(&m_tMainSimCapSet,sizeof(m_tMainSimCapSet), &tSimCapSet, sizeof(TSimCapSet)); }    
    TSimCapSet  GetMainSimCapSet( void ) const { return m_tMainSimCapSet; }

    void    SetSecondSimCapSet( TSimCapSet &tSimCapSet ) { SAFE_PATIBLE_MEMCPY(&m_tSecondSimCapSet,sizeof(m_tSecondSimCapSet), &tSimCapSet, sizeof(TSimCapSet)); }
...
public:	
	...
	void SetMediaKey(TMediaEncrypt& tMediaEncrypt){ SAFE_PATIBLE_MEMCPY(&m_tMediaKey, sizeof(m_tMediaKey),&tMediaEncrypt, sizeof(tMediaEncrypt));}
...
----------------------------------------------------------------------
	...
	04/06/28    3.0         胡昌威        修改
====================================================================*/
inline BOOL32 TMt::operator ==( const TMt & tObj ) const
{
...
inline u8 CConfId::GetConfId( u8 * pbyConfId, u8 byBufLen ) const
{
	...
	SAFE_PATIBLE_MEMCPY( pbyConfId, byBufLen, m_abyConfId, byLen );
	return( byLen );
}
...
inline u8 CConfId::SetConfId( const u8 * pbyConfId, u8 byBufLen )
{
	...
	SAFE_PATIBLE_MEMCPY( m_abyConfId, sizeof(m_abyConfId), pbyConfId, bySize );
...
inline u8 CConfId::SetConfId( LPCSTR lpszConfId )
{
	...
	{
		SAFE_PATIBLE_MEMCPY( achTemp, sizeof(achTemp),lpszTemp, 2 );
...
====================================================================*/
inline  BOOL32   TMtAlias::operator ==( const TMtAlias & tObj ) const
{
...
#endif  //_MT_H323STACKCOMPATIBLE_H