...
/* 读取配置文件类函数 */
DllExport BOOL32 GetRegKeyInt(
        const s8* lpszProfileName,  /* 文件名（含绝对路径）*/
...
/* 从配置文件中读取一整数型值 */
DllExport BOOL32 GetRegKeyStringTable(
        const s8* lpszProfileName,  /* 文件名（含绝对路径）*/
        const s8* lpszSectionName,  /* Profile中的段名   */        
...
/* 从配置文件中读取一张表里的若干行数据 */
DllExport BOOL32 GetRegKeyString(
        const s8* lpszProfileName,  /* 文件名（含绝对路径）*/
        const s8* lpszSectionName,  /* Profile中的段名   */
        const s8* lpszKeyName,      /* 段内的关键字名    */
        const s8* lpszDefault,      /* 失败时返回的默认值*/
 ...
 *
 * DllExport BOOL32 GetRegKeyString2(
 *         FILE *stream,
...
DllExport BOOL32 SetRegKeyString(
        const s8* lpszProfileName,  /* 文件名（含绝对路径）*/
        const s8* lpszSectionName,  /* Profile中的段名   */
        const s8* lpszKeyName,      /* 段内的关键字名    */
...
/*====================================================================
  ...
  输入参数说明：const s8* lpszProfileName, 文件名（含绝对路径）   
                ...
                u16 wCurInSectionOffset,当前写入的段内偏移
                         （如：写单板的时候，配置的单板过多，可能要分多次写）
                const s8* lpSzSectionKeepKey 段中要保留的关键字名
                         （如：写一整段的时候要保留entrynum的值）
  返回值说明  ：TRUE/FALSE
  ====================================================================*/
DllExport BOOL32 SetRegKeySection(
