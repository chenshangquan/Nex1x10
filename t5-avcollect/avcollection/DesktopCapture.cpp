...
bool Align4Bytes(int*pnWidth,int*pnHeight)
{
	...
	//DXGI实际采集数据无对齐方式
	if (g_emAlignBytes == ALIGN_NO_BYTES)
	{
		return false;
	}
	...
	if (g_emAlignBytes == ALIGN_4_BYTES)
	{
...
int CDesktopCapture::Create()
{
	...
	if (gdigrab_read_header(&g_avFormatatContentx))
	{
		return 1;
	}
...
bool CDesktopCapture::QueryFrame(void* pImgData, INT& nImgSize)
{
	...
	if (SUCCEEDED(hr))
	{
		INT nCaptrueImgSize = mappedRect.Pitch * m_dxgiOutDesc.DesktopCoordinates.bottom;
		if (g_bPrtSwitchOn)
		{ 
			sprintf_s(chMsg, Max_TempCharLength, "nImgSize:%d, nCaptureSize:%d, Pitch:%d\n",
				nImgSize, nCaptrueImgSize, mappedRect.Pitch);
			WriteToLog(chMsg, strnlen_s(chMsg, Max_TempCharLength));
		}

		
		// DXGI抓屏，部分型号电脑内存为4字节对齐
		if (nCaptrueImgSize != nImgSize)
		{
		...
		memcpy_s(pImgData, nImgSize, mappedRect.pBits, nImgSize);
		hStagingSurf->Unmap();
	}
